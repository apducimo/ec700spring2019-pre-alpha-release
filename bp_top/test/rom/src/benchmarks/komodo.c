...
void komodo_match_count(uint32_t comparator, data_t count, xlen_t * addr) {
  ...
  komodo_ctrl_wr(comparator, e_C_M_ADDR, addr);
}

void komodo_match_count_instruction(uint32_t comparator, data_t count,
                                    xlen_t * addr) {
  komodo_ctrl_wr(comparator, e_C_MI_COUNT, count);
  komodo_ctrl_wr(comparator, e_C_MI_ADDR, addr);
}

int64_t komodo_read_count(uint32_t comparator) {
  int64_t out;
  komodo_ctrl_rd(comparator, e_C_READ_COUNT, out);
  return out;
}

int64_t komodo_read_instruction_count(uint32_t comparator) {
  int64_t out;
  komodo_ctrl_rd(comparator, e_C_READ_INST_COUNT, out);
  return out;
}
...
void komodo_set_mem_typ(xlen_t type) {
  komodo_ctrl_wr(0, e_C_MEM_TYP, type);
}

void komodo_set_sp_base(xlen_t *addr) {
  komodo_ctrl_wr(0, e_C_SP_ADDR, addr);
}

void komodo_set_sp_offset(xlen_t *addr) {
  komodo_ctrl_wr(0, e_C_SP_OFFSET, addr);
}
...
uint64_t komodo_info_sp_base() {
  uint64_t out;
  komodo_ctrl_rd(0, e_C_INFO_SP_BASE, out);
  return out;
}
